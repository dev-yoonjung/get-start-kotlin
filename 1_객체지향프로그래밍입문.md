...
캡슐화와 더불어서 객체지향프로그래밍을 하는데 필수적인 것이 `추상화`임 -> 그만큼 제일 중요!

## 다형성이란?
- 여러 모습을 갖는 것
- 객체 지향에서는 한 객체가 여러 타입을 갖는 것
    - 즉, 한 객체가 여러 타입의 기능을 제공함
    - 타입 상속으로 다형성 구현
        - 하위 타입은 상위 타입도 됨 (다형) 
...
Timer t = it; // IotTimer는 Timer도 됨
t.start();
t.stop();

Rechargeable r  = it; // IotTimer는 Rechargeable도 됨
r.charge();
```

## 추상화란?

- 데이터나 프로세스 등을 의마가 비슷한 개념이나 의미 있는 표현으로 정의하는 과정
    - 뭔가 개념화 시키거나 표현을 도출하는 것 
- 두가지 방식의 추상화
    - 특정한 성질
    - 공통 성질(일반화)
- 예)
...
### 그러면 다형성은 추성화의 어떤 것과 관련된 것인지?

공통 성질을 뽑아내는 추상화 방식과 다형성이 관련되어 있음
